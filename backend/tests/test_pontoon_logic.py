...
def test_hit(pontoon_game):
    ...
    assert (
        len(pontoon_game.player_hand) == initial_player_hand_len + 1
    )
...
def test_is_busted_after_hit(pontoon_game):
    pontoon_game.start_game()
...
def test_dealer_turn(pontoon_game):
    ...
    assert (
        pontoon_game.get_hand_value(pontoon_game.dealer_hand) >= 17
    )
...
def test_get_game_state(pontoon_game):
    ...
    assert game_state["dealer_hand"] == [
        pontoon_game.dealer_hand[0],
        "Hidden",
    ]
    assert game_state["player_value"] == pontoon_game.get_hand_value(
        pontoon_game.player_hand
    )
    ...
    assert game_state["dealer_value"] == pontoon_game.get_hand_value(
        pontoon_game.dealer_hand
    )
